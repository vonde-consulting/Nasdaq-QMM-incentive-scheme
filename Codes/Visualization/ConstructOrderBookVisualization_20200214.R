...
##READ IN RAW DATA
file<-paste0(direc,stock,"_2013-11-",day,"_34200000_59400000_message_10.csv")
...
rm(file,cols)
##READ IN RAW DATA
file<-paste0(direc,stock,"_2013-11-",day,"_34200000_59400000_orderbook_10.csv")
Cols<-c("askprice1","asksize1","bidprice1","bidsize1","askprice2","asksize2","bidprice2","bidsize2",
        "askprice3","asksize3","bidprice3","bidsize3","askprice4","asksize4","bidprice4","bidsize4",
        "askprice5","asksize5","bidprice5","bidsize5","askprice6","asksize6","bidprice6","bidsize6",
        "askprice7","asksize7","bidprice7","bidsize7","askprice8","asksize8","bidprice8","bidsize8",
        "askprice9","asksize9","bidprice9","bidsize9","askprice10","asksize10","bidprice10","bidsize10")
mydata2<-read.csv(file=file,header=F,col.names=Cols,stringsAsFactors=F)
rm(file)
...
subplot<-data.frame(matrix(NA,0,length(cols))); colnames(subplot)<-cols
mids<-data.frame(cbind(1:length(min),  rep(NA,length(min)))); colnames(mids)<-c("interval","mid")
subold<-data.frame(matrix(NA,0,length(Cols))); colnames(subold)<-Cols
xlabels<-rep(NA,length(min))

for (y in 1:length(min)){

#find state of the order book at closest timestap before testtime
orbk<-mydata[mydata$time<=testtime,]; orbk<-orbk[nrow(orbk),c(1,8:11)]; orbk$mid<-(orbk$askprice1+orbk$bidprice1)/2
mids$mid[y]<-orbk$mid
...
execanct<-execanc[which(execanc$orderID%in%subt$orderID&execanc$time<testtime&execanc$time>=oldtesttime),]
      ...
      if (length(q<-which(subt$remainder<0))!=0){stop("Negative remainders")} #sanity check
      rm(execanct,totalect)
      }
      else {subt$remainder<-subt$size}
...
subt<-subt[which((subt$direction==1&subt$price<=orbk$bidprice1)|(subt$direction==-1&subt$price>=orbk$askprice1)),]

#aggregate across MPID type, price and direction
subt$temp<-paste(subt$mpid,subt$price,subt$direction,sep=";")
suba<-aggregate(subt$size,by=list(Category=subt$temp),FUN=sum)
s<-t(matrix(unlist(strsplit(suba$Category,split=";")),3,nrow(suba)))
suba$Category<-NULL
suba$mpid<-s[,1]
suba$price<-as.numeric(s[,2])
suba$direction<-as.integer(s[,3])
rm(s)
...
pdfname<-paste0(direc,stock,"_11-",day,"_",xlabels[1],"_",xlabels[length(xlabels)],".pdf")
pdf(file=pdfname,width=6,height=4,paper="special")
xlabels<-format(as.POSIXct((xlabels),origin="1970-01-01",tz="UTC"),"%H:%M")
p<-ggplot(subplot,aes(x=factor(interval),y=price,color=mpid))+scale_colour_manual(values=c("null"="grey","SBSH"="green","TMBR"="blue","UBSS"="red","WEMM"="pink"))+
  geom_point(alpha=1/4,aes(size=size))+scale_x_discrete(name="",breaks=seq(1,60,10),labels=xlabels[seq(1,60,10)])+
  ggtitle(paste(stock,"Nov",day))
for (i in 1:length(min)){
gg.data<-data.frame(i,mids$mid[i]); colnames(gg.data)<-c("i","mid")
p<-p+geom_point(data=gg.data,aes(x=i,y=mid),shape=3,colour="black")
rm(gg.data)
}
p
dev.off()
