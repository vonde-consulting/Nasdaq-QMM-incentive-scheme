...
consolidateVariables<-function(i){
  ...
  windows<-c(10,30,60)
    ...
    for (k in 1:length(windows)){
      ...
      mypath<-paste("Data/",sep="")
      mydata<-read.table(file=paste(mypath,ticks[i],"_2013-11-",dates[j],"_",w,"sec2.rda",sep=""))
      
      mydata$DAY<-rep(as.numeric(dates[j]),nrow(mydata))

      for (j in 2:length(dates)){
        
        mydata2<-read.table(file=paste(mypath,ticks[i],"_2013-11-",dates[j],"_",w,"sec2.rda",sep=""))
      ...
      write.table(mydata,file=paste(mypath,ticks[i],"_",w,"sec2.rda",sep=""))
