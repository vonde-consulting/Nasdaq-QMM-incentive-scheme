...
excelfilename<-paste(mypath,"SummaryStats/REGRESSOR_FULLSUMSTATS_",w,"sec.xlsx",sep="")
...
vars<-c("RELSPR","VOL.GRID.SHORT.PRE","AGGR.REL.MPID","ORSZ.DVOL.MPID","SUB.ALL.DVOL","EXE.ALL.DVOL","DEPTH.TOTAL.DVOL",
        "SUB.BUY.DVOL","EXE.BUY.DVOL","DEPTH.BUY.DVOL",
        "SUB.SELL.DVOL","EXE.SELL.DVOL","DEPTH.SELL.DVOL",
        "RELDPR.MID","HASBROUCK.MAX")
...
stats[8,]<-round(stats[8,]*(10^-6),digits=3)
stats[9,]<-round(stats[9,]*(10^-6),digits=3)
stats[10,]<-round(stats[10,]*(10^-6),digits=3)
stats[11,]<-round(stats[11,]*(10^-6),digits=3)
stats[12,]<-round(stats[12,]*(10^-6),digits=3)
stats[13,]<-round(stats[13,]*(10^-6),digits=3)
stats[14,]<-round(stats[14,]*(10^3),digits=3)
stats[15,]<-round(stats[15,]*(10^3),digits=3)

varnames<-c("Relative Bid-Ask Spreads","Volatility","MPID Aggressiveness","MPID Order Size",
            "Submissions","Executions","Depth",
            "Buy-Side Submissions","Buy-Side Executions","Buy-Side Depth",            
            "Sell-Side Submissions","Sell-Side Executions","Sell-Side Depth",
            "Price Changes","Pricing Errors")
