...
for (jj in 1:numtests){
  ...
  if (jj==1){     
    model<-RELSPR~lag(MPID.SUB,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      ...
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))|.-lag(MPID.SUB,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))
    model2<-RELSPR~lag(MPID.SUB,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1:depvarlags)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+OPEN+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+DUMMIES|.-lag(MPID.SUB,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))
  }
  if (jj==2){    
    model<-RELSPR~lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      ...
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))|.-lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))
    model2<-RELSPR~lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1:depvarlags)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1)+
      lag(DEPTH.BUY.DVOL,k=1)+OPEN+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+DUMMIES|.-lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))
  }    
  
  if (jj==3){    
    model<-RELSPR~lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      ...
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))|.-lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))
    model2<-RELSPR~lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1:depvarlags)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1)+
      lag(DEPTH.SELL.DVOL,k=1)+OPEN+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+DUMMIES|.-lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))
    
  }    
  if (jj==4){     
    model<-RELEFFSPR.EXE.REP~lag(MPID.SUB,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELEFFSPR.EXE.REP,k=1:depvarlags)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+OPEN+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))|.-lag(MPID.SUB,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))
    model2<-RELEFFSPR.EXE.REP~lag(MPID.SUB,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELEFFSPR.EXE.REP,k=1:depvarlags)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+OPEN+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+DUMMIES|.-lag(MPID.SUB,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))
  ...
  if (jj==5){     
    model<-RELRLZSPR.EXE.REP.POST~lag(MPID.SUB,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      ...
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN|.-lag(MPID.SUB,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))  
    model2<-RELRLZSPR.EXE.REP.POST~lag(MPID.SUB,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELRLZSPR.EXE.REP.PRE,k=1:depvarlags)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES|.-lag(MPID.SUB,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))  
    
  }
  ##VOLATILITY
  if (jj==6){     
    model<-VOL.GRID.SHORT.POST~lag(MPID.SUB,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1:depvarlags)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+OPEN+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))|.-lag(MPID.SUB,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))
    model2<-VOL.GRID.SHORT.POST~lag(MPID.SUB,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1:depvarlags)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+OPEN+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+DUMMIES|.-lag(MPID.SUB,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))
  ...
  if (jj==7){    
    model<-VOL.GRID.SHORT.POST~lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1:depvarlags)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1)+
      lag(DEPTH.BUY.DVOL,k=1)+OPEN+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))|.-lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))
    model2<-VOL.GRID.SHORT.POST~lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1:depvarlags)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1)+
      lag(DEPTH.BUY.DVOL,k=1)+OPEN+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+DUMMIES|.-lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))
  ...
  if (jj==8){    
    model<-VOL.GRID.SHORT.POST~lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      ...
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN|.-lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))
    model2<-VOL.GRID.SHORT.POST~lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1:depvarlags)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1)+
      lag(DEPTH.SELL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES|.-lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))
  }    
  
  if (jj==9){     
    model<-VOL.SD~lag(MPID.SUB,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.SD,k=1:depvarlags)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN|.-lag(MPID.SUB,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))
    model2<-VOL.SD~lag(MPID.SUB,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.SD,k=1:depvarlags)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES|.-lag(MPID.SUB,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))
  }
  ...
  if (jj==10){     
    model<-VOL.GRID.LONG.POST~lag(MPID.SUB,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.LONG.PRE,k=1:depvarlags)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN|.-lag(MPID.SUB,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))
    model2<-VOL.GRID.LONG.POST~lag(MPID.SUB,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.LONG.PRE,k=1:depvarlags)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES|.-lag(MPID.SUB,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))
  }
  ##SUBMISSION VOLUME
  if (jj==11){     
    model<-SUB.ALL.DVOL~lag(MPID.SUB,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1:depvarlags)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN|.-lag(MPID.SUB,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))
    model2<-SUB.ALL.DVOL~lag(MPID.SUB,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1:depvarlags)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES|.-lag(MPID.SUB,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))
  ...
  if (jj==12){    
    model<-SUB.BUY.DVOL~lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      ...
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN|.-lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))
    model2<-SUB.BUY.DVOL~lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1:depvarlags)+lag(EXE.BUY.DVOL,k=1)+
      lag(DEPTH.BUY.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES|.-lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))
    
  }    
  if (jj==13){    
    model<-SUB.SELL.DVOL~lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1:depvarlags)+lag(EXE.SELL.DVOL,k=1)+
      lag(DEPTH.SELL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN|.-lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))
    model2<-SUB.SELL.DVOL~lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1:depvarlags)+lag(EXE.SELL.DVOL,k=1)+
      lag(DEPTH.SELL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES|.-lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))
  }    
  if (jj==14){    
    model<-SUB.BUY.DVOL~lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1:depvarlags)+lag(EXE.BUY.DVOL,k=1)+
      lag(DEPTH.BUY.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN|.-lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))
    model2<-SUB.BUY.DVOL~lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1:depvarlags)+lag(EXE.BUY.DVOL,k=1)+
      lag(DEPTH.BUY.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES|.-lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))
  ...
  if (jj==15){    
    model<-SUB.SELL.DVOL~lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1:depvarlags)+lag(EXE.SELL.DVOL,k=1)+
      lag(DEPTH.SELL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN|.-lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))
    model2<-SUB.SELL.DVOL~lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1:depvarlags)+lag(EXE.SELL.DVOL,k=1)+
      lag(DEPTH.SELL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES|.-lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))
  ...
  ##EXECUTION VOLUME
  if (jj==16){     
    model<-EXE.ALL.DVOL~lag(MPID.SUB,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1:depvarlags)+
      lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN|.-lag(MPID.SUB,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))
    model2<-EXE.ALL.DVOL~lag(MPID.SUB,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1:depvarlags)+
      lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES|.-lag(MPID.SUB,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))
  ...
  if (jj==17){    
    model<-EXE.BUY.DVOL~lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1:depvarlags)+
      lag(DEPTH.BUY.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN|.-lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))
    model2<-EXE.BUY.DVOL~lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1:depvarlags)+
      lag(DEPTH.BUY.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES|.-lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))
  ...
  if (jj==18){    
    model<-EXE.SELL.DVOL~lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1:depvarlags)+
      lag(DEPTH.SELL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN|.-lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))
    model2<-EXE.SELL.DVOL~lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1:depvarlags)+
      lag(DEPTH.SELL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES|.-lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))
  ...
  if (jj==19){    
    model<-EXE.BUY.DVOL~lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1:depvarlags)+
      lag(DEPTH.BUY.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN|.-lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))
    model2<-EXE.BUY.DVOL~lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1:depvarlags)+
      lag(DEPTH.BUY.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES|.-lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))
  ...
  if (jj==20){    
    model<-EXE.SELL.DVOL~lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1:depvarlags)+
      lag(DEPTH.SELL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN|.-lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))
    model2<-EXE.SELL.DVOL~lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1:depvarlags)+
      lag(DEPTH.SELL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES|.-lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))
  }    
  ##DEPTH
  if (jj==21){     
    model<-DEPTH.TOTAL.DVOL~lag(MPID.SUB,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      ...
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN|.-lag(MPID.SUB,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))
    model2<-DEPTH.TOTAL.DVOL~lag(MPID.SUB,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1:depvarlags)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES|.-lag(MPID.SUB,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))
  ...
  if (jj==22){    
    model<-DEPTH.BUY.DVOL~lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      ...
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN|.-lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))
    model2<-DEPTH.BUY.DVOL~lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1)+
      lag(DEPTH.BUY.DVOL,k=1:depvarlags)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES|.-lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))
  ...
  if (jj==23){    
    model<-DEPTH.SELL.DVOL~lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      ...
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN|.-lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))
    model2<-DEPTH.SELL.DVOL~lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1)+
      lag(DEPTH.SELL.DVOL,k=1:depvarlags)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES|.-lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))
  ...
  if (jj==24){    
    model<-DEPTH.BUY.DVOL~lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1)+
      lag(DEPTH.BUY.DVOL,k=1:depvarlags)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN|.-lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))
    model2<-DEPTH.BUY.DVOL~lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1)+
      lag(DEPTH.BUY.DVOL,k=1:depvarlags)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES|.-lag(MPID.SUB.SELL,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))
  ...
  if (jj==25){    
    model<-DEPTH.SELL.DVOL~lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1)+
      lag(DEPTH.SELL.DVOL,k=1:depvarlags)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN|.-lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))
    model2<-DEPTH.SELL.DVOL~lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1)+
      lag(DEPTH.SELL.DVOL,k=1:depvarlags)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES|.-lag(MPID.SUB.BUY,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))
  }   
