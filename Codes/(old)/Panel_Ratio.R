...
for (jj in 1:numtests){
  ...
  if (jj==1){     #negative dummy
    model<-MPID.SUB.RATIO~lag(MPID.SUB.RATIO,k=c(1:mpidlags))+AGGR.REL.MPID+ORSZ.DVOL.MPID+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+
  ...
  if (jj==2){    #positive dummy
    model<-MPID.SUB.RATIO~lag(MPID.SUB.RATIO,k=c(1:mpidlags))+AGGR.REL.MPID+ORSZ.DVOL.MPID+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+
      ...
      (lag(abs(RELDPR.MID),k=1)*lag(POS.DUMMY,k=1))+OPEN
  }      
  if (jj==3){    #negative dummy, buy side on buy side
    model<-MPID.SUB.BUY.RATIO~lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+AGGR.REL.MPID+ORSZ.DVOL.MPID+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+
  ...
  if (jj==7){    #negative dummy, sell side on sell side
    model<-MPID.SUB.SELL.RATIO~lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+AGGR.REL.MPID+ORSZ.DVOL.MPID+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1)+
      lag(DEPTH.SELL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN
  }  
  ...
  if (jj==15){     #just total
    model<-MPID.SUB.RATIO~lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)
  }
  if (jj==16){    #just buy side
    model<-MPID.SUB.BUY.RATIO~lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)
  }      
  if (jj==17){    #just sell side
    model<-MPID.SUB.SELL.RATIO~lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)
  }  
  if (jj==18){     #negative dummy
    model<-MPID.SUB.RATIO~lag(MPID.SUB.RATIO,k=c(1:mpidlags))+AGGR.REL.MPID+ORSZ.DVOL.MPID+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+OPEN
  }    
  if (jj==19){     #negative dummy
    model<-MPID.SUB.RATIO~lag(MPID.SUB.RATIO,k=c(1:mpidlags))+AGGR.REL.MPID+ORSZ.DVOL.MPID+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+HASBROUCK.MAX
  }
