...
for (jj in 26:numtests){
  ...
  ##HASBROUCK
  if (jj==26){     
    model<-HASBROUCK.MAX~lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+lag(SUB.ALL.NUM,k=1:mpidlags)+lag(HASBROUCK.MAX,k=1:depvarlags)
    model2<-HASBROUCK.MAX~lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
  ...
  if (jj==27){    
    model<-HASBROUCK.MAX~lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1)+
      lag(DEPTH.BUY.DVOL,k=1:depvarlags)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+lag(SUB.BUY.NUM,k=1:mpidlags)+lag(HASBROUCK.MAX,k=1:depvarlags)
    model2<-HASBROUCK.MAX~lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1)+
      lag(DEPTH.BUY.DVOL,k=1:depvarlags)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
  ...
  if (jj==28){    
    model<-HASBROUCK.MAX~lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1)+
      lag(DEPTH.SELL.DVOL,k=1:depvarlags)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+lag(SUB.SELL.NUM,k=1:mpidlags)+lag(HASBROUCK.MAX,k=1:depvarlags)
    model2<-HASBROUCK.MAX~lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1)+
      lag(DEPTH.SELL.DVOL,k=1:depvarlags)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
