...
#options(error=recover)
  ...
  for (jj in 1:numtests){
    ...
    if (jj==1){     
      model<-RELSPR~lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        ...
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+lag(SUB.ALL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))-lag(SUB.ALL.NUM,k=1:mpidlags)+lag(SUB.ALL.NUM.INST,k=1:mpidlags)
      model2<-RELSPR~lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1:depvarlags)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
        lag(DEPTH.TOTAL.DVOL,k=1)+OPEN+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+DUMMIES+lag(SUB.ALL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))-lag(SUB.ALL.NUM,k=1:mpidlags)+lag(SUB.ALL.NUM.INST,k=1:mpidlags)
    }
    if (jj==2){    
      model<-RELSPR~lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        ...
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+lag(SUB.BUY.NUM,k=1:mpidlags)|.-lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))-lag(SUB.BUY.NUM,k=1:mpidlags)+lag(SUB.BUY.NUM.INST,k=1:mpidlags)
      model2<-RELSPR~lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1:depvarlags)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1)+
        lag(DEPTH.BUY.DVOL,k=1)+OPEN+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+DUMMIES+lag(SUB.BUY.NUM,k=1:mpidlags)|.-lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))-lag(SUB.BUY.NUM,k=1:mpidlags)+lag(SUB.BUY.NUM.INST,k=1:mpidlags)
    }    
    
    if (jj==3){    
      model<-RELSPR~lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        ...
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+lag(SUB.SELL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))-lag(SUB.SELL.NUM,k=1:mpidlags)+lag(SUB.SELL.NUM.INST,k=1:mpidlags)
      model2<-RELSPR~lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1:depvarlags)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1)+
        lag(DEPTH.SELL.DVOL,k=1)+OPEN+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+DUMMIES+lag(SUB.SELL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))-lag(SUB.SELL.NUM,k=1:mpidlags)+lag(SUB.SELL.NUM.INST,k=1:mpidlags)
      
    }    
    if (jj==4){     
      model<-RELEFFSPR.EXE.REP~lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELEFFSPR.EXE.REP,k=1:depvarlags)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
        lag(DEPTH.TOTAL.DVOL,k=1)+OPEN+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+lag(SUB.ALL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))-lag(SUB.ALL.NUM,k=1:mpidlags)+lag(SUB.ALL.NUM.INST,k=1:mpidlags)
      model2<-RELEFFSPR.EXE.REP~lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELEFFSPR.EXE.REP,k=1:depvarlags)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
        lag(DEPTH.TOTAL.DVOL,k=1)+OPEN+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+DUMMIES+lag(SUB.ALL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))-lag(SUB.ALL.NUM,k=1:mpidlags)+lag(SUB.ALL.NUM.INST,k=1:mpidlags)
    ...
    if (jj==5){     
      model<-RELRLZSPR.EXE.REP.POST~lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        ...
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+lag(SUB.ALL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))-lag(SUB.ALL.NUM,k=1:mpidlags)+lag(SUB.ALL.NUM.INST,k=1:mpidlags)  
      model2<-RELRLZSPR.EXE.REP.POST~lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELRLZSPR.EXE.REP.PRE,k=1:depvarlags)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
        lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES+lag(SUB.ALL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))-lag(SUB.ALL.NUM,k=1:mpidlags)+lag(SUB.ALL.NUM.INST,k=1:mpidlags)
      
    }
    ##VOLATILITY
    if (jj==6){     
      model<-VOL.GRID.SHORT.POST~lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1:depvarlags)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
        lag(DEPTH.TOTAL.DVOL,k=1)+OPEN+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+lag(SUB.ALL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))-lag(SUB.ALL.NUM,k=1:mpidlags)+lag(SUB.ALL.NUM.INST,k=1:mpidlags)
      model2<-VOL.GRID.SHORT.POST~lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1:depvarlags)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
        lag(DEPTH.TOTAL.DVOL,k=1)+OPEN+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+DUMMIES+lag(SUB.ALL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))-lag(SUB.ALL.NUM,k=1:mpidlags)+lag(SUB.ALL.NUM.INST,k=1:mpidlags)
    ...
    if (jj==7){    
      model<-VOL.GRID.SHORT.POST~lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1:depvarlags)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1)+
        lag(DEPTH.BUY.DVOL,k=1)+OPEN+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+lag(SUB.BUY.NUM,k=1:mpidlags)|.-lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))-lag(SUB.BUY.NUM,k=1:mpidlags)+lag(SUB.BUY.NUM.INST,k=1:mpidlags)
      model2<-VOL.GRID.SHORT.POST~lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1:depvarlags)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1)+
        lag(DEPTH.BUY.DVOL,k=1)+OPEN+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+DUMMIES+lag(SUB.BUY.NUM,k=1:mpidlags)|.-lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))-lag(SUB.BUY.NUM,k=1:mpidlags)+lag(SUB.BUY.NUM.INST,k=1:mpidlags)
    ...
    if (jj==8){    
      model<-VOL.GRID.SHORT.POST~lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1:depvarlags)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1)+
        lag(DEPTH.SELL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)++lag(SUB.SELL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))-lag(SUB.SELL.NUM,k=1:mpidlags)+lag(SUB.SELL.NUM.INST,k=1:mpidlags)
      model2<-VOL.GRID.SHORT.POST~lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1:depvarlags)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1)+
        lag(DEPTH.SELL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES+lag(SUB.SELL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))-lag(SUB.SELL.NUM,k=1:mpidlags)+lag(SUB.SELL.NUM.INST,k=1:mpidlags)
    }    
    
    if (jj==9){     
      model<-VOL.SD~lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.SD,k=1:depvarlags)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
        lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+lag(SUB.ALL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))-lag(SUB.ALL.NUM,k=1:mpidlags)+lag(SUB.ALL.NUM.INST,k=1:mpidlags)
      model2<-VOL.SD~lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.SD,k=1:depvarlags)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
        lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES+lag(SUB.ALL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))-lag(SUB.ALL.NUM,k=1:mpidlags)+lag(SUB.ALL.NUM.INST,k=1:mpidlags)
    }
    ...
    if (jj==10){     
      model<-VOL.GRID.LONG.POST~lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.LONG.PRE,k=1:depvarlags)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
        lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+lag(SUB.ALL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))-lag(SUB.ALL.NUM,k=1:mpidlags)+lag(SUB.ALL.NUM.INST,k=1:mpidlags)
      model2<-VOL.GRID.LONG.POST~lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.LONG.PRE,k=1:depvarlags)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
        lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES+lag(SUB.ALL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))-lag(SUB.ALL.NUM,k=1:mpidlags)+lag(SUB.ALL.NUM.INST,k=1:mpidlags)
    }
    ##SUBMISSION VOLUME
    if (jj==11){     
      model<-SUB.ALL.DVOL~lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1:depvarlags)+lag(EXE.ALL.DVOL,k=1)+
        lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+lag(SUB.ALL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))-lag(SUB.ALL.NUM,k=1:mpidlags)+lag(SUB.ALL.NUM.INST,k=1:mpidlags)
      model2<-SUB.ALL.DVOL~lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1:depvarlags)+lag(EXE.ALL.DVOL,k=1)+
        lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES+lag(SUB.ALL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))-lag(SUB.ALL.NUM,k=1:mpidlags)+lag(SUB.ALL.NUM.INST,k=1:mpidlags)
    ...
    if (jj==12){    
      model<-SUB.BUY.DVOL~lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        ...
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+lag(SUB.BUY.NUM,k=1:mpidlags)|.-lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))-lag(SUB.BUY.NUM,k=1:mpidlags)+lag(SUB.BUY.NUM.INST,k=1:mpidlags)
      model2<-SUB.BUY.DVOL~lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1:depvarlags)+lag(EXE.BUY.DVOL,k=1)+
        lag(DEPTH.BUY.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES+lag(SUB.BUY.NUM,k=1:mpidlags)|.-lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))-lag(SUB.BUY.NUM,k=1:mpidlags)+lag(SUB.BUY.NUM.INST,k=1:mpidlags)
      
    }    
    if (jj==13){    
      model<-SUB.SELL.DVOL~lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1:depvarlags)+lag(EXE.SELL.DVOL,k=1)+
        lag(DEPTH.SELL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+lag(SUB.SELL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))-lag(SUB.SELL.NUM,k=1:mpidlags)+lag(SUB.SELL.NUM.INST,k=1:mpidlags)
      model2<-SUB.SELL.DVOL~lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1:depvarlags)+lag(EXE.SELL.DVOL,k=1)+
        lag(DEPTH.SELL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES+lag(SUB.SELL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))-lag(SUB.SELL.NUM,k=1:mpidlags)+lag(SUB.SELL.NUM.INST,k=1:mpidlags)
    }    
    if (jj==14){    
      model<-SUB.BUY.DVOL~lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1:depvarlags)+lag(EXE.BUY.DVOL,k=1)+
        lag(DEPTH.BUY.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+lag(SUB.SELL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))-lag(SUB.SELL.NUM,k=1:mpidlags)+lag(SUB.SELL.NUM.INST,k=1:mpidlags)
      model2<-SUB.BUY.DVOL~lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1:depvarlags)+lag(EXE.BUY.DVOL,k=1)+
        lag(DEPTH.BUY.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES+lag(SUB.SELL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))-lag(SUB.SELL.NUM,k=1:mpidlags)+lag(SUB.SELL.NUM.INST,k=1:mpidlags)
    ...
    if (jj==15){    
      model<-SUB.SELL.DVOL~lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1:depvarlags)+lag(EXE.SELL.DVOL,k=1)+
        lag(DEPTH.SELL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+lag(SUB.BUY.NUM,k=1:mpidlags)|.-lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))-lag(SUB.BUY.NUM,k=1:mpidlags)+lag(SUB.BUY.NUM.INST,k=1:mpidlags)
      model2<-SUB.SELL.DVOL~lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1:depvarlags)+lag(EXE.SELL.DVOL,k=1)+
        lag(DEPTH.SELL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES+lag(SUB.BUY.NUM,k=1:mpidlags)|.-lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))-lag(SUB.BUY.NUM,k=1:mpidlags)+lag(SUB.BUY.NUM.INST,k=1:mpidlags)
    ...
    ##EXECUTION VOLUME
    if (jj==16){     
      model<-EXE.ALL.DVOL~lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1:depvarlags)+
        lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+lag(SUB.ALL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))-lag(SUB.ALL.NUM,k=1:mpidlags)+lag(SUB.ALL.NUM.INST,k=1:mpidlags)
      model2<-EXE.ALL.DVOL~lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1:depvarlags)+
        lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES+lag(SUB.ALL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))-lag(SUB.ALL.NUM,k=1:mpidlags)+lag(SUB.ALL.NUM.INST,k=1:mpidlags)
    ...
    if (jj==17){    
      model<-EXE.BUY.DVOL~lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1:depvarlags)+
        lag(DEPTH.BUY.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+lag(SUB.BUY.NUM,k=1:mpidlags)|.-lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))-lag(SUB.BUY.NUM,k=1:mpidlags)+lag(SUB.BUY.NUM.INST,k=1:mpidlags)
      model2<-EXE.BUY.DVOL~lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1:depvarlags)+
        lag(DEPTH.BUY.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES+lag(SUB.BUY.NUM,k=1:mpidlags)|.-lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))-lag(SUB.BUY.NUM,k=1:mpidlags)+lag(SUB.BUY.NUM.INST,k=1:mpidlags)
    ...
    if (jj==18){    
      model<-EXE.SELL.DVOL~lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1:depvarlags)+
        lag(DEPTH.SELL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+lag(SUB.SELL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))-lag(SUB.SELL.NUM,k=1:mpidlags)+lag(SUB.SELL.NUM.INST,k=1:mpidlags)
      model2<-EXE.SELL.DVOL~lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1:depvarlags)+
        lag(DEPTH.SELL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES+lag(SUB.SELL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))-lag(SUB.SELL.NUM,k=1:mpidlags)+lag(SUB.SELL.NUM.INST,k=1:mpidlags)
    ...
    if (jj==19){    
      model<-EXE.BUY.DVOL~lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1:depvarlags)+
        lag(DEPTH.BUY.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+lag(SUB.SELL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))-lag(SUB.SELL.NUM,k=1:mpidlags)+lag(SUB.SELL.NUM.INST,k=1:mpidlags)
      model2<-EXE.BUY.DVOL~lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1:depvarlags)+
        lag(DEPTH.BUY.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES+lag(SUB.SELL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))-lag(SUB.SELL.NUM,k=1:mpidlags)+lag(SUB.SELL.NUM.INST,k=1:mpidlags)
    ...
    if (jj==20){    
      model<-EXE.SELL.DVOL~lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1:depvarlags)+
        lag(DEPTH.SELL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+lag(SUB.BUY.NUM,k=1:mpidlags)|.-lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))-lag(SUB.BUY.NUM,k=1:mpidlags)+lag(SUB.BUY.NUM.INST,k=1:mpidlags)
      model2<-EXE.SELL.DVOL~lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1:depvarlags)+
        lag(DEPTH.SELL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES+lag(SUB.BUY.NUM,k=1:mpidlags)|.-lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))-lag(SUB.BUY.NUM,k=1:mpidlags)+lag(SUB.BUY.NUM.INST,k=1:mpidlags)
    }    
    ##DEPTH
    if (jj==21){     
      model<-DEPTH.TOTAL.DVOL~lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        ...
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+lag(SUB.ALL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))-lag(SUB.ALL.NUM,k=1:mpidlags)+lag(SUB.ALL.NUM.INST,k=1:mpidlags)
      model2<-DEPTH.TOTAL.DVOL~lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
        lag(DEPTH.TOTAL.DVOL,k=1:depvarlags)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES+lag(SUB.ALL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.INST,k=c(1:mpidlags))-lag(SUB.ALL.NUM,k=1:mpidlags)+lag(SUB.ALL.NUM.INST,k=1:mpidlags)
    }
    if (jj==22){    
      model<-DEPTH.BUY.DVOL~lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        ...
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+lag(SUB.BUY.NUM,k=1:mpidlags)|.-lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))-lag(SUB.BUY.NUM,k=1:mpidlags)+lag(SUB.BUY.NUM.INST,k=1:mpidlags)
      model2<-DEPTH.BUY.DVOL~lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1)+
        lag(DEPTH.BUY.DVOL,k=1:depvarlags)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES+lag(SUB.BUY.NUM,k=1:mpidlags)|.-lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))-lag(SUB.BUY.NUM,k=1:mpidlags)+lag(SUB.BUY.NUM.INST,k=1:mpidlags)
    ...
    if (jj==23){    
      model<-DEPTH.SELL.DVOL~lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        ...
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+lag(SUB.SELL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))-lag(SUB.SELL.NUM,k=1:mpidlags)+lag(SUB.SELL.NUM.INST,k=1:mpidlags)
      model2<-DEPTH.SELL.DVOL~lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1)+
        lag(DEPTH.SELL.DVOL,k=1:depvarlags)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES+lag(SUB.SELL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))-lag(SUB.SELL.NUM,k=1:mpidlags)+lag(SUB.SELL.NUM.INST,k=1:mpidlags)
    ...
    if (jj==24){    
      model<-DEPTH.BUY.DVOL~lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1)+
        lag(DEPTH.BUY.DVOL,k=1:depvarlags)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+lag(SUB.SELL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))-lag(SUB.SELL.NUM,k=1:mpidlags)+lag(SUB.SELL.NUM.INST,k=1:mpidlags)
      model2<-DEPTH.BUY.DVOL~lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1)+
        lag(DEPTH.BUY.DVOL,k=1:depvarlags)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES+lag(SUB.SELL.NUM,k=1:mpidlags)|.-lag(MPID.SUB.SELL.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))-lag(SUB.SELL.NUM,k=1:mpidlags)+lag(SUB.SELL.NUM.INST,k=1:mpidlags)
    ...
    if (jj==25){    
      model<-DEPTH.SELL.DVOL~lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1)+
        lag(DEPTH.SELL.DVOL,k=1:depvarlags)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+lag(SUB.BUY.NUM,k=1:mpidlags)|.-lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))-lag(SUB.BUY.NUM,k=1:mpidlags)+lag(SUB.BUY.NUM.INST,k=1:mpidlags)
      model2<-DEPTH.SELL.DVOL~lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(AGGR.REL.TOTAL,k=1)+lag(ORSZ.DVOL.TOTAL,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
        lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1)+
        lag(DEPTH.SELL.DVOL,k=1:depvarlags)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
        (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+DUMMIES+lag(SUB.BUY.NUM,k=1:mpidlags)|.-lag(MPID.SUB.BUY.RATIO,k=c(1:mpidlags))+lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))-lag(SUB.BUY.NUM,k=1:mpidlags)+lag(SUB.BUY.NUM.INST,k=1:mpidlags)
    }     
    ...
    #test1<-plmtest(fit1,effect="time", type="bp")
    ...
    #model2<-update(model, . ~ . + DUMMIES)
    cat(paste("\n \n","Firm+Day Fixed Regression #", jj, ", Dep Var: ",all.vars(model)[1],"\n \n",sep=""),file=filename,append=TRUE)
    fit4<-plm(model2,data=mydata,model="pooling",index=c("TICK","TIMEPOINT"))
    s<-summary(fit4)
    capture.output(s,file=filename,append=T)    
    #clustered errors
    #s<-coeftest(fit4, vcov=vcovHC(fit4,type="HC0",cluster="group"))
    #capture.output(s, file=filename,append=T)
    dw1<-pdwtest(fit4,alternative="two.sided")
    capture.output(dw1, file=filename,append=T)
    
    filename<-paste(mypath,"Results/", folder, "/Panel_Regression_Post_",w,"sec_",jj,".rda",sep="")
    save(fit1,fit2,fit3,fit4,file=filename)
    
    #model<-RELSPR~lag(RELSPR,k=1)+OPEN|lag(RELDPR.MID,k=1)+OPEN
    #X <- model.matrix(model, mydata, rhs = 1, model = "within")
    #W <- model.matrix(model, mydata, rhs = 2, model = "within")
    #ht<-pht(model,data=mydata, model="ht",index=c("TICK","TIMEPOINT"))
