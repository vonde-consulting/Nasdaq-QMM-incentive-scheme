...
nms<-c("MPID.SUB.RATIO","MPID.SUB.BUY.RATIO","MPID.SUB.SELL.RATIO")
...
for (kk in 1:length(nms)){
  ...
  for (vv in 1:length(ticks)){
    ...
    #pdf(paste(mypath,"/Graphs/Regressors/TMBR/",ticks[vv],"_",nms[kk],"_Ratio_",w,"sec.pdf",sep=""))
...
filename<-paste(mypath,"Results/", folder, "/TMBR_REGRESSOR_STATIONARITY_BYFIRM_",w,"sec.xlsx",sep="")
...
#  for (l in 1:20){
  ...
  for (jj in 1:numregs){
    ...
    filename<-paste(mypath,"Results/", folder, "/TMBR_Regression_Ratio_",w,"sec_",jj,".txt",sep="")
    ...
    v<-unname(residuals(fit))
        ...
        filename2<-paste(mypath,"Results/", folder, "/TMBR_Regression_Ratio_",ticks[ii],"_",w,"sec_",jj,".rda",sep="")
      ...
      write.xlsx(formatlab, paste(mypath,"Results/", folder, "/TMBR_Regression_Ratio_",w,"sec_",jj,".xlsx",sep=""))
  ...
  filename<-paste(mypath,"Results/", folder, "/TMBR_INDIVIDUALERRORS_",lags,"Lags_",w,"sec.xlsx",sep="")
  write.xlsx(stationary,file=filename,append=FALSE) 
