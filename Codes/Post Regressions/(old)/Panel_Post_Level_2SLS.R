...
folder<-"Panel_Post_Level_TMBR_2SLS"

k<-3
w<-windows[k]

mypath<-paste(head,"Dropbox/Projects/Trader Anonymity/Data/",sep="")
#filename<-paste(mypath,"DATA_LEVELS_INST_",w,"sec.Rds",sep="")
filename<-paste(mypath,"DATA_TMBR_LEVELS_INST_",w,"sec.Rds",sep="")
load(file=filename)
...
diagnostics<-matrix(NA,numtests,6)
mpidlags<-1
depvarlags<-5
...
for (jj in 1:numtests){
  ...
  if (jj==1){     
    ...
    reg1<-MPID.SUB~lag(MPID.SUB.INST,k=c(1:mpidlags))+lag(HASBROUCK.MAX,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      ...
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))
    
    model<-RELSPR~lag(MPID.SUB.HAT,k=c(1:mpidlags))+lag(HASBROUCK.MAX,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1:depvarlags)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+OPEN+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))
  ...
  if (jj==2){    
    ...
    reg1<-MPID.SUB.BUY~lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))+lag(HASBROUCK.MAX,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
  ...
  if (jj==3){    
    ...
    reg1<-MPID.SUB.SELL~lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))+lag(HASBROUCK.MAX,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
  ...
  if (jj==4){    
    ...
    model<-RELEFFSPR.EXE.REP~lag(MPID.SUB.HAT,k=c(1:mpidlags))+lag(HASBROUCK.MAX,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELEFFSPR.EXE.REP,k=1:depvarlags)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+OPEN+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))
  ...
  if (jj==5){     
    ...
    model<-RELRLZSPR.EXE.REP.POST~lag(MPID.SUB.HAT,k=c(1:mpidlags))+lag(HASBROUCK.MAX,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELRLZSPR.EXE.REP.PRE,k=1:depvarlags)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      ...
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN 
  ...
  if (jj==6){   
    ...
    model<-VOL.GRID.SHORT.POST~lag(MPID.SUB.HAT,k=c(1:mpidlags))+lag(HASBROUCK.MAX,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1:depvarlags)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+OPEN+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))
  ...
  if (jj==9){     
    ...
    model<-VOL.SD~lag(MPID.SUB.HAT,k=c(1:mpidlags))+lag(HASBROUCK.MAX,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.SD,k=1:depvarlags)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN
  ...
  if (jj==10){     
    ...
    model<-VOL.GRID.LONG.POST~lag(MPID.SUB.HAT,k=c(1:mpidlags))+lag(HASBROUCK.MAX,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.LONG.PRE,k=1:depvarlags)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN
  ...
  if (jj==11){     
    ...
    model<-SUB.ALL.DVOL~lag(MPID.SUB.HAT,k=c(1:mpidlags))+lag(HASBROUCK.MAX,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1:depvarlags)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN
  ...
  if (jj==12){    
    ...
    model<-SUB.BUY.DVOL~lag(MPID.SUB.HAT,k=c(1:mpidlags))+lag(HASBROUCK.MAX,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1:depvarlags)+lag(EXE.BUY.DVOL,k=1)+
  ...
  if (jj==13){    
    ...
    model<-SUB.SELL.DVOL~lag(MPID.SUB.HAT,k=c(1:mpidlags))+lag(HASBROUCK.MAX,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1:depvarlags)+lag(EXE.SELL.DVOL,k=1)+
      lag(DEPTH.SELL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN
  ...
  if (jj==16){     
    ...
    model<-EXE.ALL.DVOL~lag(MPID.SUB.HAT,k=c(1:mpidlags))+lag(HASBROUCK.MAX,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1:depvarlags)+
      lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN
  ...
  if (jj==17){    
    ...
    model<-EXE.BUY.DVOL~lag(MPID.SUB.HAT,k=c(1:mpidlags))+lag(HASBROUCK.MAX,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1:depvarlags)+
      lag(DEPTH.BUY.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN
  ...
  if (jj==18){   
    ...
    model<-EXE.SELL.DVOL~lag(MPID.SUB.HAT,k=c(1:mpidlags))+lag(HASBROUCK.MAX,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1:depvarlags)+
      lag(DEPTH.SELL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN
  ...
  if (jj==21){    
    ...
    model<-DEPTH.TOTAL.DVOL~lag(MPID.SUB.HAT,k=c(1:mpidlags))+lag(HASBROUCK.MAX,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
  ...
  if (jj==22){    
    ...
    model<-DEPTH.BUY.DVOL~lag(MPID.SUB.HAT,k=c(1:mpidlags))+lag(HASBROUCK.MAX,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1)+
  ...
  if (jj==23){    
    ...
    model<-DEPTH.SELL.DVOL~lag(MPID.SUB.HAT,k=c(1:mpidlags))+lag(HASBROUCK.MAX,k=1)+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1)+
  ...
  if (jj==26){    
    ...
    reg1<-MPID.SUB~lag(MPID.SUB.INST,k=c(1:mpidlags))+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+OPEN+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+lag(HASBROUCK.MAX,k=1:depvarlags)
    
    model<-HASBROUCK.MAX~lag(MPID.SUB.HAT,k=c(1:mpidlags))+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+OPEN+lag(HASBROUCK.MAX,k=1:depvarlags)
    
  }  
  if (jj==27){  
    
    reg1<-MPID.SUB.BUY~lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1)+
      lag(DEPTH.BUY.DVOL,k=1)+OPEN+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+lag(HASBROUCK.MAX,k=1:depvarlags)
  ...
  if (jj==28){    
    reg1<-MPID.SUB.SELL~lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1)+
      lag(DEPTH.SELL.DVOL,k=1)+OPEN+lag(abs(RELDPR.MID),k=1)+lag(NEG.DUMMY,k=1)+
      (lag(abs(RELDPR.MID),k=1)*lag(NEG.DUMMY,k=1))+lag(HASBROUCK.MAX,k=1:depvarlags)
  ...
  if (jj==29){    
    ...
    reg1<-MPID.SUB~lag(MPID.SUB.INST,k=c(1:mpidlags))+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+OPEN+lag(HASBROUCK.MAX,k=1)+lag(RELDPR.MID,k=1:depvarlags)

    model<-RELDPR.MID~lag(MPID.SUB.HAT,k=c(1:mpidlags))+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+lag(HASBROUCK.MAX,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.ALL.DVOL,k=1)+lag(EXE.ALL.DVOL,k=1)+
      lag(DEPTH.TOTAL.DVOL,k=1)+OPEN+lag(RELDPR.MID,k=1:depvarlags)
  ...
  if (jj==30){  
    ...
    reg1<-MPID.SUB.BUY~lag(MPID.SUB.BUY.INST,k=c(1:mpidlags))+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.BUY.DVOL,k=1)+lag(EXE.BUY.DVOL,k=1)+
      lag(DEPTH.BUY.DVOL,k=1)+OPEN+lag(HASBROUCK.MAX,k=1)+lag(RELDPR.MID,k=1:depvarlags)
    model<-RELDPR.MID~lag(MPID.SUB.HAT,k=c(1:mpidlags))+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      ...
      OPEN+lag(RELDPR.MID,k=1:depvarlags)    
  }    
  if (jj==31){    
    
    reg1<-MPID.SUB.SELL~lag(MPID.SUB.SELL.INST,k=c(1:mpidlags))+lag(AGGR.REL.MPID,k=1)+lag(ORSZ.DVOL.MPID,k=1)+
      lag(VOL.GRID.SHORT.PRE,k=1)+lag(RELSPR,k=1)+lag(SUB.SELL.DVOL,k=1)+lag(EXE.SELL.DVOL,k=1)+
      lag(DEPTH.SELL.DVOL,k=1)+OPEN+lag(HASBROUCK.MAX,k=1)+lag(RELDPR.MID,k=1:depvarlags)
  ...
  fit<-plm(reg1,data=mydata,model="pooling",index=c("TICK","TIMEPOINT")); k<-length(names(fit$coefficients))
    ...
    m<-update(reg1,.~.-lag(MPID.SUB.INST,k=c(1:mpidlags)))}
    if (jj%in%c(2,7,12,15,17,20,22,25,27)==1){
    m<-update(reg1,.~.-lag(MPID.SUB.BUY.INST,k=c(1:mpidlags)))}
    if (jj%in%c(3,8,13,14,18,19,23,24,28)==1){
    m<-update(reg1,.~.-lag(MPID.SUB.SELL.INST,k=c(1:mpidlags)))}  
  ...
  rm(MPID.SUB.HAT)
  q<-which(colnames(mydata)%in%c("MPID.SUB.HAT"))
  mydata<-mydata[,-q]
  ...
  diagnostics[jj,]<-cbind(Fstat1,pval1,Fstat3,pval3,lgtest$statistic,lgtest$p.value)
